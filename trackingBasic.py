...
# lam cho video phat dung toc do
fps = cap.get(cv2.CAP_PROP_FPS) #toc do khung hinh/s cua video
wait_time = 1000/fps #thoi gian cho cua khung hinh theo ms
print fps

play = True
term_crit = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 1)
while True:
    pre_time = time.time() # thoi gian hien tai cua khung hinh

    if play:
        ret, img = cap.read() #img la khung hinh cua video
    if img is None:
        img = temp_img
    else:
        temp_img = img
    ...
    if Mouse_event.img is not None:
        ...
        #
        contours, ret1 = cv2.findContours(edge, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        mask = np.zeros((roi_img.shape), np.uint8)

        for e in contours:
            #xac dinh cac duong vien
            e = cv2.convexHull(e)
    ...
    delta_time = (time.time() - pre_time) * 1000 #thoi gian xu ly anh
    if delta_time > wait_time: # xu ly qua lau, lau hon thoi gian cho khung hinh
        delay_time = 1
    else:
